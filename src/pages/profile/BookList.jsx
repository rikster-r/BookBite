...
const BookList = ({ books, title, sort, openModal, username }) => {
  ...
  return (
    <div>
      <h2 className="pl-6 mb-2 text-lg font-medium">{title}</h2>
      <div>
        {filtered.map((book) => {
          return (
            <div
              key={book.id}
              className="relative group-one flex items-center p-3 pr-6 gap-3 hover:bg-gray-800 hover:text-white text-base"
            >
              {/* full image on hover */}
              <img
                src={book.imageUrl}
              ...
              <h3 className="mr-auto line-clamp-2 sm:font-medium font-semibold">
                {book.title}
              </h3>
              {book.notes && (
              ...
              {book.rating && (
                <p className="whitespace-nowrap ml-1 text-sm sm:text-base">
